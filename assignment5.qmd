...
acs_map_clust <- acs_map %>%
  ...
  mutate(cluster = factor(cluster_assignments))  # Add cluster assignments

#mean of the variables used for clustering by cluster
...
acs_il_t <- acs_il_t %>%
  mutate(
    name = tolower(gsub("census tract [0-9.]+, | county|illinois", "", county)),
...
```{r}
# Clean data for clustering census tracts
...
# We will iterate over K_values and apply means_cluster for each value of K, storing the results in a list
...
colnames(final_data)[(ncol(final_data) - length(K_values) + 1):ncol(final_data)] <- paste0("cluster_K", K_values)
```
